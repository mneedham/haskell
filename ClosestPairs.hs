...
bfClosest pairs = 
	snd $ last $ 
	    let x  = scanl (\ acc@(min, soFar) (p1, p2) -> 
			             if distance p1 p2 < min then (distance p1 p2, Just(Pair p1 p2)) else acc) 
...
data Pair a = Pair (Point a) (Point a) 
...
dcClosest pairs  
	...
	| otherwise = 		    
		fst $ last $ scanl (\(closest, shouldExit) (p1,p2) -> 
							  let currentClosest = minimumBy (compare `on` distance') [closest, Pair p1 p2] in
...
dcClosest2 :: (Ord a, Floating a) => [Point a] -> Pair a
dcClosest2 pairs
  ...
  where sortedByX = sortBy compare pairs	      
        ...
        closestPair = minimumBy (compare `on` distance') [closestLeftPair, closestRightPair]
          where closestLeftPair =  dcClosest2 leftByX
...
distance' (Pair (x1, y1) (x2, y2)) =  sqrt $ ((x1 - x2) ^ 2) + ((y1 - y2) ^ 2)	 
